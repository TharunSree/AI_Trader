...
def papertrading_view(request):
    # --- NEW: Logic to find all trained models ---
    model_dir = Path("saved_models")
    model_files = []
    if model_dir.exists():
        model_files = [f.name for f in model_dir.iterdir() if f.suffix == '.pth']

    # We will assume only one PaperTrader instance for now
    trader, created = PaperTrader.objects.get_or_create(id=1)

    context = {
        'model_files': model_files,
        'trader_status': trader.status,
        'account_equity': "100,000.00"  # Placeholder
    }
