...
broker_html = '''  <!-- Broker Accounts Configuration -->
  <div class="glass rounded-2xl p-6 relative overflow-hidden flex flex-col md:col-span-2">
    <h3 class="text-sm font-medium text-slate-400 mb-6 tracking-widest uppercase">Broker Account Modules</h3>
    
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
      {% for account in broker_accounts %}
      <div class="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4 flex justify-between items-center">
          <div>
          ...
          <form method="post" action="{% url 'delete_broker_account_api' account.id %}" class="inline">
              {% csrf_token %}
      ...
      <div class="col-span-1 md:col-span-2 text-slate-500 font-mono text-sm text-center py-4 bg-slate-900/30 rounded-xl border border-slate-800/50">
          No secondary broker accounts linked. System defaults to master .env configuration.
      </div>
      {% endfor %}
    </div>
...
target = "<!-- Database Management Tool -->"
if target in text:
    text = text.replace(target, broker_html + "\n\n  " + target)
