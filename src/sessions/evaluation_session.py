...
class EvaluationSession:
    ...
    def _resolve_model_config(self):
        ...
        if is_database_model_reference(model_reference):
            ...
            # Robust auto-healing: If the model's ticker in the database is 'SPY' but the name is 'Meta Best - ...',
            # it was generated by the Meta Trainer on their remote server before the backend patch.
            # We auto-heal the ticker value to 'ALL_CRYPTO' dynamically!
            if ticker == 'SPY' and job.name and "Meta Best" in job.name:
                logger.info(f"Auto-healing model {model_reference} ticker from SPY to ALL_CRYPTO (Meta Best model detected).")
                ticker = 'ALL_CRYPTO'
